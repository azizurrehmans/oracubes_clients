...
--------------------------------------------------------
  ...
  SELECT bpi.customer_name bank_voc_customer, i.account_no, i.account_type, i.bank_date, i.bank_id, i.customer_id, p.eform_amount, nvl(i.payment_in_fc,0) - nvl(p.eform_amount,0) balance,
     ...
     FROM exp_payment i, ac_coa_view c, ac_voucher_det v, exp_customer, exp_bank_pi bpi,
    ...
    WHERE i.customer_id = c.coa_seq(+)
      ...
      ---
      and i.inv_payment_id = bpi.inv_payment_id(+)
;
